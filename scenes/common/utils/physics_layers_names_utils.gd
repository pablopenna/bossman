...
func get_collision_layer_from_name(layer_name: StringName) -> int:
	if not _layers_dict.has(layer_name):
		push_error("Collision layer named %s does not exist" % layer_name)
	return _layers_dict[layer_name]
